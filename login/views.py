...
def index(request):
    ...
    return render(request, 'login/index.html', {'errors': errors, 'error_reason': error_reason, 'not_auth_warn': not_auth_warn, 'success_slo': success_slo,
...
def attrs(request):
    ...
    if 'samlUserdata' in request.session:
        ...
        if len(request.session['samlUserdata']) > 0:
            attributes = request.session['samlUserdata'].items()
    return render(request, 'login/attrs.html',
